...
namespace Frost.Utility
{
	...
	/// <typeparam name="TValue">Type of object to cache (must implement <see cref="IDisposable"/>)</typeparam>
	...
	public class Cache<TKey, TValue> : IDictionary<TKey, TValue> where TValue : class
	{
		...
		/// </summary>
		private const float HotRate = 0.95f;
		#endregion
		...
		public ICollection<TValue> Values
		{
			get
			{
				lock(_cacheEntries)
				{
					...
					list.AddRange(_cacheEntries.Values.Select(item => item.Value));
					return list;
				}
			}
		}
		...
		public IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator ()
		{
			lock(_cacheEntries)
			{
				...
				list.AddRange(_cacheEntries.Select(entry => new KeyValuePair<TKey, TValue>(entry.Key, entry.Value.Value)));
		...
		/// <summary>
		/// Contains the value of the entry and its status
		/// </summary>
		private class CacheEntry
		{
			...
			/// </summary>
			/// <returns>A string in the form: Key = Value [Status]</returns>
